...
A Bun/TypeScript CLI that installs upstream pstack skills/agents onto a host (Cursor, Claude Code, Codex, Gemini) without rewriting skill bodies. The CLI lives in `shim/` and is launched via `bin/pstack-anywhere` (a `#!/usr/bin/env bun` shim).
...
- Commands (see `shim/package.json` scripts and the CI workflow):
  ...
  - Run the app: `./bin/pstack-anywhere <command>` from the repo root (`install --host <cursor|claude-code|codex|gemini> [--scope user|project] [--dry-run]`, `uninstall`, `sync`, `doctor --host <id>`, `status`, `emit-plugin`).
